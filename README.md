...
- **`virtualenv python not found at .venv\Scripts\python.exe`**: The `.venv` folder was almost certainly created on **Linux/WSL** (`bin/python`), not Windows (`Scripts\python.exe`). The Windows launcher detects that and **recreates** `.venv` for Windows. If it still fails, delete `.venv` in the repo and run `scripts\demo-day.cmd -SetupOnly` again.
