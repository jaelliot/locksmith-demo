...
- **`virtualenv python not found at .venv\Scripts\python.exe`**: The `.venv` folder was almost certainly created on **Linux/WSL** (`bin/python`), not Windows (`Scripts\python.exe`). The Windows launcher detects that and **recreates** `.venv` for Windows. If removal fails under `\\wsl.localhost\...`, the script falls back to **`wsl rm -rf`**. You can also delete `.venv` yourself from WSL (`rm -rf .venv`) and rerun `scripts\demo-day.cmd -SetupOnly`.
